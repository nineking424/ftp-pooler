...
│  │   Kafka Cluster  │                                                    │
│  │  ┌────────────┐  │     ┌──────────────────────────────────────────┐  │
│  │  │ ftp-tasks  │──┼────▶│         FTP Pooler StatefulSet           │  │
│  │  └────────────┘  │     │  ┌────────────────────────────────────┐  │  │
│  │  ┌────────────┐  │     │  │           Application               │  │  │
│  │  │ftp-results │◀─┼─────│  │  ┌──────────┐    ┌──────────────┐  │  │  │
...
┌─────────────────────────────────────────────────────────────────────┐
│                        Application Layer                             │
│  main.py: 라이프사이클 관리, 컴포넌트 조율, 신호 처리                  │
└─────────────────────────────────────────────────────────────────────┘
                                   │
        ┌──────────────────────────┼──────────────────────────┐
        ▼                          ▼                          ▼
┌───────────────┐        ┌─────────────────┐        ┌───────────────┐
│    Config     │        │    Transfer     │        │     API       │
│   settings.py │        │   engine.py     │        │   routes.py   │
│ connections.py│        │   task.py       │        │               │
└───────────────┘        └─────────────────┘        └───────────────┘
        │                          │                          │
        │                          ▼                          │
        │                ┌─────────────────┐                  │
        ...
        ▼                          ▼                          ▼
┌───────────────┐        ┌─────────────────┐        ┌───────────────┐
│    Kafka      │        │    External     │        │    Metrics    │
│  consumer.py  │        │   aioftp/FTP    │        │ prometheus.py │
│  producer.py  │        │                 │        │               │
└───────────────┘        └─────────────────┘        └───────────────┘
```
...
```
Level 1: Pod-Wide Limit (max_sessions_per_pod: 100)
    │
    └── Semaphore in SessionPoolManager
        │
        Level 2: Per-Connection Limit (max_sessions_per_connection: 10)
            │
            └── Semaphore in SessionPool
                │
                Level 3: Session State Machine
                    │
                    └── IDLE → BUSY → IDLE/ERROR/DISCONNECTED
```
...
Layer 4: Application (main.py)
    │ Signal handlers, top-level exception logging
    │
Layer 3: Transfer Engine (engine.py)
    │ Comprehensive catch-all with error classification
    │ Error codes: INVALID_CONFIG, FILE_NOT_FOUND, CONNECTION_ERROR, IO_ERROR, UNKNOWN_ERROR
    │
Layer 2: Pool Manager (manager.py)
    │ Graceful degradation, session reconnection
    │
Layer 1: Session (session.py)
    │ Connection state management, health checks
```
...
│  Phase 1: 안정성 강화                                                    │
│  ════════════════════                                                    │
│  ├─ [ ] Kafka 재시도 로직 (Exponential Backoff)                         │
│  ├─ [ ] Dead Letter Queue 구현                                          │
│  ├─ [ ] 전송 타임아웃 (asyncio.timeout)                                  │
│  └─ [ ] Circuit Breaker 패턴 도입                                        │
│                                                                          │
│  Phase 2: 성능 최적화                                                    │
│  ════════════════════                                                    │
│  ├─ [ ] 비동기 Producer (send_and_wait → async queue)                   │
│  ├─ [ ] 커넥션 풀 예열 (Pre-warming)                                     │
│  ├─ [ ] 배치 처리 최적화                                                 │
│  └─ [ ] Consumer lag 모니터링                                            │
│                                                                          │
│  Phase 3: 보안 강화                                                      │
│  ════════════════════                                                    │
│  ├─ [ ] FTPS (FTP over TLS) 지원                                        │
│  ├─ [ ] SFTP (SSH) 지원                                                  │
│  ├─ [ ] API 인증 (JWT/mTLS)                                              │
│  └─ [ ] 감사 로그 강화                                                   │
│                                                                          │
